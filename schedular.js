...
class CPUSchedular {
    constructor(tasks, schedular_type) {
        ...
        if(!Array.isArray(tasks) ){
            throw new Error("Tasks can not be empty")
        }
        if(!schedular_type){
            throw new Error("Specify schedular type (PRIORITY || STACK || QUEUE  )")
        }
        if(!SCHEDULAR_TYPE[schedular_type]){
            throw new Error("Schedular type must be either of the following (PRIORITY || STACK || QUEUE  )")
        }
        ...
        if(tasks.length >0){
            for (let i = 0; i < tasks.length; i++) {
                if (schedular_type === SCHEDULAR_TYPE.PRIORITY) if (!tasks[i].priority) throw new Error(`priority key not found on a task ${i + 1} while using schedular type: Priority`)
                if(!tasks[i].process){
                    throw new Error(`key: process  not found on a task ${i + 1}`)
                }
    ...
    addNewCPUTask(task){
        if(!task){
            throw new Error(`No task is added`)
        }
        if(!task.process){
            throw new Error(`key: process  not found on a task`)
        }
        if (this.schedular_type === SCHEDULAR_TYPE.PRIORITY) if (!task.priority) throw new Error(`priority key not found on this task while using schedular type: Priority`)
        this.SCHEDULAR_DECISOR.addNewCPUTask(task)
    }
}
